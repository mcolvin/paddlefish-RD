...
ch<- read.csv("C:/Users/mcolvin/Documents/projects/Paddlefish/analysis/paddlefish-RD/big-jamma.csv")


names(ch)
indx<-grep("_0",names(ch))

socc<-cbind(indx,c(indx[-1]-1 ,ncol(ch)))

capturehistories<- lapply(nrow(socc),x,
    {
    out<- ch[socc[x,1],socc[x,2]]
    })
