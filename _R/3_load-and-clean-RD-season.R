...
dat$tt<-sort(unique(effort$date-(min(effort$date)-1)))
...
dat$ch<-as.matrix(chipm)
dat$ch[dat$ch>1]<-1 ## multiple lengths
colnames(dat$ch)<-NULL
dat$ch<- rbind(dat$ch,matrix(0,20,ncol(dat$ch))) ## AUGMENT BY 20 ROWS
...
dat$X[,5]<-scale(dat$X[,5],center=mean(dat$X[,5]),scale=sd(dat$X[,5]))
...
saveRDS(dat,"_output/rd-dat-season.RDS")